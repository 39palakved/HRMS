...
const AttendanceManager = () => {
  ...
  useEffect(() => {
    const fetchEmployees = async () => {
      try {
        const res = await axios.get("https://hrms-backend-pink.vercel.app/api/employees");
  ...
  const handleMark = async (employeeId, status) => {
    try {
      await axios.post("https://hrms-backend-pink.vercel.app/api/attendance", {
        employeeId,
        date,
        status
      });
  ...
  const toggleReport = async () => {
    if (!showReport) {
      ...
      try {
        const res = await axios.get(`https://hrms-backend-pink.vercel.app/api/attendance?date=${date}`);
